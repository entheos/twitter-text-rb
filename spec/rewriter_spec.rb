...
describe TwitterText::Rewriter do
  ...
  describe "rewrite usernames" do #{{{
    before do
      @rewritten_text = TwitterText::Rewriter.rewrite_usernames_or_lists(original_text, &method(:block))
    end
  ...
  describe "rewrite hashtags" do #{{{
    before do
      @rewritten_text = TwitterText::Rewriter.rewrite_hashtags(original_text, &method(:block))
    end
  ...
  describe "rewrite urls" do #{{{
    ...
    before do
      @rewritten_text = TwitterText::Rewriter.rewrite_urls(original_text, &method(:block))
    end
    ...
    context "with a URL ending in allowed punctuation" do
      it "does not consume ending punctuation" do
        %w| ? ! , . : ; ] ) } = \ ' |.each do |char|
          TwitterText::Rewriter.rewrite_urls("#{url}#{char}") do |url|
    ...
    context "with a URL preceded in forbidden characters" do
      it "should be rewritten" do
        %w| \ ' / ! = |.each do |char|
          TwitterText::Rewriter.rewrite_urls("#{char}#{url}") do |url|
